...
describe('uniform mode', function () {
  var LIMIT = 10;
  var KEY = 'ioredis-ratelimit:test:uniform';
  ...
  it('should the total size beyond the limit when the bucket size is not enough for batch actions', function (done) {
    this.slow(500);
    this.timeout(3000);
    ...
    co(function* () {
      ...
      yield limiter(BATCH_SIZE).then(function (actual) {
        assert.deepEqual(actual, { total: total + BATCH_SIZE, acknowledged: BATCH_SIZE, remaining: 0 });
      });
  ...
  it('should throw an error when the bucket is full', function (done) {
    this.slow(500);
    this.timeout(3000);
